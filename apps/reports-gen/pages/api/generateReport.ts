...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    console.log('Quering the backend with :', report_url);
    ...
    try {
        ...
        const response = await fetch(`https://sundai-heron.tk.co/generate_report?report_url=${encodeURIComponent(report_url)}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        if (!response.ok) {
            const errorData = /* read response body in plain text */ await response.text();
